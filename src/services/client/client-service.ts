...
export type { RestaurantList, LocalList, DeliveryPointCredentials, DeliveryPoint, ClientDish, Cart, Order, OrderHistoryStatus, OrderRating, OrderRatingValue, OrderRequest, PaymentResponse };
...
interface RestaurantDtoFromApi {
    ...
    nombre: string;
    urlFoto: string | null;
    estadoServicio?: boolean | string | null;
...
function mapRestaurantDtoApiToRestaurantType(r: RestaurantDtoFromApi): RestaurantList {
    return {
        ...
        state: mapRestaurantServiceState(r.estadoServicio),
    };
}

function mapRestaurantServiceState(value: boolean | string | null | undefined): boolean {
    if (typeof value === "boolean") return value;
    if (typeof value !== "string") return false;

    const normalized = value.trim().toUpperCase();
    return ["ACTIVO", "ABIERTO", "TRUE", "S", "SI", "1"].includes(normalized);
}
...
async function hydrateOrdersWithLocalRatings(
    ...
    const ordersToHydrate = orders.filter(
        (order) => order.estado === "FINALIZADO" && !order.calificacionLocal,
    );
...
export async function submitOrderLocalRating(
    ...
    try {
        const response = await clientApi.post<OrderRatingDtoFromApi>(
            `/api/clientes/${session.idTipoUsuario}/pedidos/${orderId}/calificacion-local`,
            body,
